    ...
    try {
      if (key == "cfg_file_path") {
        ...
        if (val == "true" || val == "1") {
          ...
          throw std::runtime_error(
              "leg_only must be one of {true,false,1,0}");
        }
      } else if (key == "arm_enabled") {
        if (val == "true" || val == "1") {
          arm_enabled_ = true;
        } else if (val == "false" || val == "0") {
          arm_enabled_ = false;
          ...
          throw std::runtime_error(
              "arm_enabled must be one of {true,false,1,0}");
        }
      } else if (key == "waist_enabled") {
        if (val == "true" || val == "1") {
          waist_enabled_ = true;
        } else if (val == "false" || val == "0") {
          waist_enabled_ = false;
        } else {
          throw std::runtime_error(
              "waist_enabled must be one of {true,false,1,0}");
        }
      } else if (key == "waist_zero") {
        if (val == "true" || val == "1") {
          waist_zero_ = true;
        } else if (val == "false" || val == "0") {
          waist_zero_ = false;
          ...
          throw std::runtime_error(
              "waist_zero must be one of {true,false,1,0}");
        }
      } else {
...
bool A3AimrtBackend::RegisterPubSub_() {
  ...
  try {
    ...
    if (publish_enabled_) {
      ...
      pub_opts.leg_only = leg_only_;
      pub_opts.arm_enabled = arm_enabled_;
      pub_opts.waist_enabled = waist_enabled_;
      pub_opts.waist_zero = waist_zero_;
      ...
      if (waist_enabled_) {
        ...
        if (!aimrt::channel::RegisterPublishType<joint_msgs::msg::JointCommand>(waist_pub))
          throw std::runtime_error("register waist publish failed");
        auto waist_proxy = std::make_shared<aimrt::channel::PublisherProxy<joint_msgs::msg::JointCommand>>(waist_pub);
        pub_opts.waist_publish_fn = [waist_proxy](const joint_msgs::msg::JointCommand& m) { waist_proxy->Publish(m); };
      }
      if (!leg_only_) {
        auto neck_pub  = ch.GetPublisher("/body_drive/neck_joint_command");
        if (!aimrt::channel::RegisterPublishType<joint_msgs::msg::JointCommand>(neck_pub))
          throw std::runtime_error("register neck publish failed");
        auto neck_proxy = std::make_shared<aimrt::channel::PublisherProxy<joint_msgs::msg::JointCommand>>(neck_pub);
        pub_opts.neck_publish_fn = [neck_proxy](const joint_msgs::msg::JointCommand& m) { neck_proxy->Publish(m); };
      }
      if (arm_enabled_) {
        auto arm_pub = ch.GetPublisher("/body_drive/arm_joint_command");
        if (!aimrt::channel::RegisterPublishType<joint_msgs::msg::JointCommand>(arm_pub))
          throw std::runtime_error("register arm publish failed");
        auto arm_proxy = std::make_shared<aimrt::channel::PublisherProxy<joint_msgs::msg::JointCommand>>(arm_pub);
        pub_opts.arm_publish_fn = [arm_proxy](const joint_msgs::msg::JointCommand& m) { arm_proxy->Publish(m); };
      }
      pub_mgr_ =
