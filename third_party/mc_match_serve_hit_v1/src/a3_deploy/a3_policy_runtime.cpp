...
class OrtCpuPolicyRuntime final : public A3PolicyRuntime {
  ...
  bool Initialize(const std::string& model_path,
    ...
    try {
      ...
      if (input_count != 1 && input_count != 2) {
        std::cerr << "A3 policy must have 1 or 2 inputs, got "
                  << session_->GetInputCount() << "\n";
        return false;
      }
      if (session_->GetOutputCount() < 1) {
        std::cerr << "A3 policy must have at least 1 output\n";
        return false;
      }
      ...
      if (input_name_ != "obs_dict" && input_name_ != "obs_raw" && input_name_ != "obs") {
        std::cerr << "A3 policy input tensor must be 'obs_dict', 'obs_raw' or 'obs', got '"
                  << input_name_ << "'\n";
        return false;
      }
      has_time_step_ = input_count == 2;
      if (has_time_step_) {
        auto time_name = session_->GetInputNameAllocated(1, allocator_);
        if (std::string(time_name.get()) != "time_step") {
          std::cerr << "A3 policy second input must be 'time_step', got '"
                    << time_name.get() << "'\n";
          return false;
        }
      }
      ...
      for (std::size_t i = 0; i < session_->GetOutputCount(); ++i) {
        auto candidate = session_->GetOutputNameAllocated(i, allocator_);
        if (std::string(candidate.get()) == "action" ||
            std::string(candidate.get()) == "actions") {
          output_index_ = i;
          break;
        }
      }
      auto output_name =
          session_->GetOutputNameAllocated(output_index_, allocator_);
      output_name_ = output_name.get();
      if (output_name_ != "action" && output_name_ != "actions") {
        std::cerr << "A3 policy output tensor must be 'action' or 'actions', got '"
                  << output_name_ << "'\n";
        return false;
      }
      ...
      const auto output_type_info = session_->GetOutputTypeInfo(output_index_);
      ...
      time_step_buffer_.assign(1, 0.0f);
      ...
      if (has_time_step_) {
        time_step_shape_ = {1, 1};
        input_tensors_.emplace_back(Ort::Value::CreateTensor<float>(
            *memory_info_, time_step_buffer_.data(), time_step_buffer_.size(),
            time_step_shape_.data(), time_step_shape_.size()));
      }
  ...
  bool Infer() override {
    ...
    try {
      ...
      if (has_time_step_) {
        ...
        time_step_buffer_[0] = static_cast<float>(inference_step_++);
      }
      const char* output_names[] = {output_name_.c_str()};
      Ort::RunOptions run_options;
      session_->Run(run_options, input_names.data(), input_tensors_.data(),
                    input_tensors_.size(), output_names,
                    &output_tensor_, 1);
 ...
 private:
  ...
  std::vector<float> time_step_buffer_;
  std::vector<int64_t> time_step_shape_;
  bool has_time_step_{false};
  std::size_t output_index_{0};
  std::uint64_t inference_step_{0};
