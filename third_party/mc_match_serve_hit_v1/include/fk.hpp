/**
 * @file fk.hpp
 * @brief Forward kinematics (FK) for the A3 robot, parsed from a MuJoCo XML model.
 *
 * RobotFK loads a MuJoCo MJCF XML file (e.g. `a3/a3_29dof.xml`), extracts the
 * kinematic tree (joint axes, translations, rest rotations), and provides a
 * `DoFK()` method that computes world-frame positions and orientations for
 * every body given a root pose and joint angles.
 *
 * The FK result is used by MotionSequence::ComputeFK() to populate body-part
 * positions and quaternions from joint-angle data.
 */

#pragma once

#include <string>
#include <vector>
#include <array>

struct XMLNode;

/**
 * @class RobotFK
 * @brief Forward kinematics solver for the A3 robot, initialised from a MuJoCo XML.
 */
class RobotFK
{
    public:

        RobotFK(const std::string &xmlfile);

        void DoFK(
            std::array<double, 3> *positions_world,
            std::array<double, 4> *rotations_world,
            const std::array<double, 3> &root_translation,
            const std::array<double, 4> &root_rotation,
            const double *joint_angles
        ) const;

        // A3-native path for the full 31-DOF MuJoCo body order. The XML
        // contains a free-joint pelvis root followed by SDK slots [0..30].
        void DoFKA3(
            std::array<double, 3> *positions_world,
            std::array<double, 4> *rotations_world,
            const std::array<double, 3> &root_translation,
            const std::array<double, 4> &root_rotation,
            const double *joint_angles_31
        ) const;

        int NumJoints() const { return node_children_.size(); }
        int FindBodyIndex(const std::string &name) const;
    
    private:

        void FKChildren(
            std::array<double, 3> *positions_world,
            std::array<double, 4> *rotations_world,
            int parent_idx,
            const double *joint_angles
        ) const;

        void FKChildrenA3(
            std::array<double, 3> *positions_world,
            std::array<double, 4> *rotations_world,
            int parent_idx,
            const double *joint_angles_31
        ) const;

        void AddNode(XMLNode *node);

        std::vector< std::array<double, 3> > axes_;
        std::vector< std::array<double, 3> > translations_;
        std::vector< std::array<double, 4> > rest_rotations_;
        std::vector< std::vector<int> > node_children_;
        std::vector< std::string > node_names_;
        // SDK slot selected by the body's joint name. Body traversal order is
        // tree order (legs are visited before the torso), so child_idx - 1 is
        // not a valid A3 joint index.
        std::vector<int> a3_joint_indices_;
};
