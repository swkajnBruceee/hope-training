...
def _pick_indices(data: np.lib.npyio.NpzFile, config: dict[str, Any], limit: int | None) -> list[int]:
    ...
    if per_stroke > 0:
        for label in preferred:
            group = sorted([x for x in candidates if x[1] == label], key=lambda x: (-x[2], x[0]))
            for idx, _, _ in group[:per_stroke]:
                selected.append(idx)
                used.add(idx)
    remaining = sorted([x for x in candidates if x[0] not in used], key=lambda x: (-x[2], x[0]))
    for idx, _, _ in remaining:
        if len(selected) >= first_batch:
            break
        selected.append(idx)
    return selected[:first_batch]
...
def main() -> None:
    ...
    selected = _pick_indices(data, config, args.limit)
    samples = []
    for idx in selected:
