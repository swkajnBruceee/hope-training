...
For a headless verification video:

```bash
MUJOCO_GL=egl python -m a3_deploy_onnx_ref_pingpong \
    --config ../config/hope_pingpong_runtime.yaml \
    --max-ticks 500 \
    --video ../logs/mujoco_verification/hope_model21800_fixed_station_demo_10s.mp4
```

Flags include `--backend {mujoco,aimrt}`, `--onnx`, `--model-xml`, `--view`,
`--realtime`, `--video`, `--duration N` / `--max-ticks N`, `--idle`,
`--auto-leg-hold`, `--leg-clamp-rad`, `--leg-smooth-alpha`, and the native
warmup/gain overrides.
...
1. run the native-style PD_STAND warmup and policy handoff;
2. read robot state from the sim bridge;
3. poll the latest `RacketCommand`; gate the target, derive a fixed world station,
   wait for station readiness, and advance the swing lifecycle;
4. assemble the 110-D observation (raw, no normalization);
5. run the ONNX actor → `raw_action[31]`;
6. clip raw action to +/-20, then zero the passive head columns (idx 3, 4) to form the **applied action** and feed
   that back as the next `last_action` (matching training's zeroed feedback);
7. map the applied action → 31 joint targets via the shared ActionAdapter (holding the
   passive neck at its default);
8. apply optional native leg clamp/EMA, group gains, fall protection, and the
   post-swing official static handoff;
9. write the targets and step the sim.

`task_id`/`task_revision` semantics: a new
