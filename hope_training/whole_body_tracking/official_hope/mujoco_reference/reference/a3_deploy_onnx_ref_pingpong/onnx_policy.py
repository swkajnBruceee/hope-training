...
class OnnxPolicy:
    def __init__(self, onnx_path: str | Path, providers: list[str] | None = None) -> None:
        ...
        # Hitter-pure deploy geometry.  These are not optional conveniences: the
        # native pp_policy uses them to derive the world station and to reject
        # planner requests outside the distribution used by the actor.
        self._hp_pos_boxes = self._parse_rows(
            meta.get("hitter_pure_pos_range_per_clip", ""), 6
        )
        self._hp_vel_boxes = self._parse_rows(
            meta.get("hitter_pure_vel_range_per_clip", ""), 6
        )
        self._hp_vel_core_boxes = self._parse_rows(
            meta.get("hitter_pure_vel_core_range_per_clip", ""), 6
        )
        self._hp_vel_planner_boxes = self._parse_rows(
            meta.get("hitter_pure_vel_planner_range_per_clip", ""), 6
        )
        self._base_target_range = self._parse_flat(
            meta.get("hitter_pure_base_target_range", ""), 4
        )
        self._reach_offsets = self._parse_flat(
            meta.get("ref_reach_offset_xy", ""), 4
        )
        fixed_plane = meta.get("hitter_pure_fixed_plane_x", "")
        self.fixed_plane_x = float(fixed_plane) if fixed_plane else None
    ...
    @staticmethod
    def _parse_flat(raw: str, count: int):
        if not raw:
            return None
        try:
    ...
    @classmethod
    def _parse_rows(cls, raw: str, width: int):
        if not raw:
            return None
        rows = []
        try:
            for row in raw.split(";"):
                values = tuple(float(item) for item in row.split(","))
                if len(values) != width:
                    return None
                rows.append(values)
        except ValueError:
    ...
    @property
    def hitter_pure_pos_boxes(self):
        return self._hp_pos_boxes

    @property
    def hitter_pure_vel_boxes(self):
        return self._hp_vel_boxes

    @property
    def hitter_pure_vel_core_boxes(self):
        return self._hp_vel_core_boxes

    @property
    def hitter_pure_vel_planner_boxes(self):
        return self._hp_vel_planner_boxes

    @property
    def base_target_range(self):
        return self._base_target_range

    @property
    def reach_offsets(self):
        return self._reach_offsets
