...
class MujocoDirectBridge(SimBridge):
    def __init__(
        ...
        if video_path is not None:
            ...
            # The shipped MJCF leaves MuJoCo's offscreen framebuffer at its
            # conservative default (640x480).  Enlarge it for verification
            # renders without changing the physical model.
            self.model.vis.global_.offwidth = max(int(video_width), int(self.model.vis.global_.offwidth))
            self.model.vis.global_.offheight = max(int(video_height), int(self.model.vis.global_.offheight))
    ...
    def reset(self) -> None:
        ...
        if additive_stance is None and joints is not None:
            ...
            for name, value in joints.items():
                ...
                self.data.qpos[self._q_adr[i]] = float(value)
            self.data.qpos[self._base_qadr + 2] += float(root_height_delta)
...
class AimrtSimBridge(SimBridge):
    ...
    def record_frame(self) -> None:  # pragma: no cover - seam
        ...
