...
def main():
    ...
    out=Path(args.output_dir) if args.output_dir else ROOT/"results"/"model21800_v2B"/f"B21_action_feasibility_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    for action in tier1_actions():
        if not search: print("B21_PHASE=FIRST_CASE_BEGIN",flush=True)
        search.append(run_case(len(search),1,action))
        if search[-1]["LEGAL_RETURN"]: break
    witness=first_legal_result(search)
    if witness is None:
        for action in tier2_additional_actions():
            search.append(run_case(len(search),2,action))
            if search[-1]["LEGAL_RETURN"]: break
        witness=first_legal_result(search)
    repeats=[]
