 ...
 *
 * The A3 robot reports per-motor fault codes via the `motorstate()` field in
...
class ErrorMonitor {
 public:
   ...
   */
  void update(const std::array<uint32_t, A3_POLICY_DOF>& motorstates) {
    for (int i = 0; i < A3_POLICY_DOF; ++i) {
 ...
 private:
  std::array<uint32_t, A3_POLICY_DOF> prev_motorstate_;
  int error_count_ = 0;
  ...
  static const char* jointName(int index) {
    ...
    if (index >= 0 && index < A3_POLICY_DOF) return names[index];
    return "Unknown";
  }
};
