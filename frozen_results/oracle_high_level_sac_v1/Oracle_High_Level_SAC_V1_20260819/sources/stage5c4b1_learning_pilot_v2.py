...
def main():
    ...
    B2_EPISODES = int(os.environ.get("STAGE5C3_PILOT_TRAIN_EPISODES", "6"))


    # =========================================================
    # STAGE5C3-C3-B FORMAL TRAINER INFRASTRUCTURE
    # =========================================================

    if B2_EPISODES < 2:

        raise RuntimeError(
            "Formal SAC pilot requires "
            "at least 2 training episodes"
        )


    stage5c3_c3_output_env = (
        os.environ.get(
            "STAGE5C3_PILOT_OUTPUT_DIR"
        )
    )


    if not stage5c3_c3_output_env:
    ...
    stage5c3_c3_output_dir = Path(
        stage5c3_c3_output_env
    ).expanduser().resolve()


    stage5c3_c3_output_dir.mkdir(
        parents=True,
        exist_ok=True,
    )


    stage5c3_c3_checkpoint_dir = (
        stage5c3_c3_output_dir
        / "checkpoints"
    )

    stage5c3_c3_checkpoint_dir.mkdir(
        parents=True,
        exist_ok=True,
    )


    STAGE5C3_C3_CHECKPOINT_EVERY_UPDATES = int(
        os.environ.get(
            "STAGE5C3_CHECKPOINT_EVERY_UPDATES",
            "2",
        )
    )
    ...
    if (
        STAGE5C3_C3_CHECKPOINT_EVERY_UPDATES
        <= 0
    ):
    ...
    stage5c3_c3_training_metrics = []

    stage5c3_c3_checkpoint_paths = []


    print(
        "STAGE5C3_C3_TRAIN_EPISODES=",
        B2_EPISODES,
    )
    ...
    print(
        "STAGE5C3_C3_TRAINING_MODE="
        "STOCHASTIC_SAC_ACTOR"
    )

    print(
        "STAGE5C3_C3_DETERMINISTIC_EVALUATION="
        "SEPARATE_NEXT_STAGE"
    )
    ...
    # =========================================================
    # STAGE5C3-C2 SAC TRAINER
    #
    # First stage that performs real gradient-based updates.
    #
    # Contextual-bandit / one-step terminal transition:
    #
    #   obs -> action -> full physical strike -> reward -> done
    #
    # Every collected transition is terminal.
    # =========================================================
    ...
    stage5c3_sac_q1_target.load_state_dict(
        stage5c3_sac_q1.state_dict()
    )

    stage5c3_sac_q2_target.load_state_dict(
        stage5c3_sac_q2.state_dict()
    )


    for parameter in (
        stage5c3_sac_q1_target.parameters()
    ):
    ...
    for parameter in (
        stage5c3_sac_q2_target.parameters()
    ):
    ...
    # Compatibility alias for inherited C1 diagnostics.
    stage5c3_c1_policy = (
        stage5c3_sac_actor
    )
    ...
    stage5c3_sac_q1_initial_parameters = [
        parameter
        .detach()
        .cpu()
        .clone()
        for parameter
        in stage5c3_sac_q1.parameters()
    ]

    stage5c3_sac_q2_initial_parameters = [
        parameter
        .detach()
        .cpu()
        .clone()
        for parameter
        in stage5c3_sac_q2.parameters()
    ]

    stage5c3_sac_q1_target_initial_parameters = [
        parameter
        .detach()
        .cpu()
        .clone()
        for parameter
        in stage5c3_sac_q1_target.parameters()
    ]
    ...
    stage5c3_sac_actor_optimizer = (
        torch.optim.Adam(
            stage5c3_sac_actor.parameters(),
            lr=3.0e-4,
        )
    )

    stage5c3_sac_q1_optimizer = (
        torch.optim.Adam(
            stage5c3_sac_q1.parameters(),
            lr=3.0e-4,
        )
    )

    stage5c3_sac_q2_optimizer = (
        torch.optim.Adam(
            stage5c3_sac_q2.parameters(),
            lr=3.0e-4,
        )
    )


    stage5c3_sac_log_alpha = torch.tensor(
        math.log(
            0.20
        ),
        device=device,
    ...
    stage5c3_sac_initial_log_alpha = float(
        stage5c3_sac_log_alpha
        .detach()
        .cpu()
        .item()
    )
    ...
    STAGE5C3_SAC_GAMMA = 0.99
    STAGE5C3_SAC_TAU = 0.005
    STAGE5C3_SAC_BATCH_SIZE = 32
    STAGE5C3_SAC_TARGET_ENTROPY = -4.0
    STAGE5C3_SAC_REPLAY_CAPACITY = 8192


    stage5c3_sac_replay_obs = np.zeros(
        (
            STAGE5C3_SAC_REPLAY_CAPACITY,
            6,
        ),
        dtype=np.float32,
    )

    stage5c3_sac_replay_action = np.zeros(
        (
            STAGE5C3_SAC_REPLAY_CAPACITY,
            4,
        ),
    ...
    stage5c3_sac_replay_reward = np.zeros(
        (
            STAGE5C3_SAC_REPLAY_CAPACITY,
            1,
        ),
    ...
    stage5c3_sac_replay_next_obs = np.zeros(
        (
            STAGE5C3_SAC_REPLAY_CAPACITY,
            6,
        ),
    ...
    stage5c3_sac_replay_done = np.zeros(
        (
            STAGE5C3_SAC_REPLAY_CAPACITY,
            1,
        ),
    ...
    stage5c3_sac_replay_ptr = 0
    stage5c3_sac_replay_size = 0


    stage5c3_sac_update_steps = 0
    stage5c3_sac_q1_optimizer_steps = 0
    stage5c3_sac_q2_optimizer_steps = 0
    stage5c3_sac_actor_optimizer_steps = 0
    stage5c3_sac_alpha_optimizer_steps = 0


    stage5c3_sac_q1_losses = []
    stage5c3_sac_q2_losses = []
    stage5c3_sac_actor_losses = []
    stage5c3_sac_alpha_losses = []


    # Compatibility counters:
    #
    # In C1 these were required to remain zero.
    # In C2 they are expected to become > 0.
    ...
    # =========================================================
    # STAGE5C3-C3 CHECKPOINT HELPERS
    # =========================================================

    def stage5c3_c3_cpu_state_dict(
        module,
    ):
    ...
    def stage5c3_c3_checkpoint_payload(
        completed_episode,
    ):
        ...
        return {
            ...
            "replay_size":
                int(
                    stage5c3_sac_replay_size
                ),

            "replay_ptr":
                int(
                    stage5c3_sac_replay_ptr
                ),

            "sac_update_steps":
                int(
                    stage5c3_sac_update_steps
                ),

            "q1_optimizer_steps":
                int(
                    stage5c3_sac_q1_optimizer_steps
                ),

            "q2_optimizer_steps":
                int(
                    stage5c3_sac_q2_optimizer_steps
                ),

            "actor_optimizer_steps":
                int(
                    stage5c3_sac_actor_optimizer_steps
                ),

            "alpha_optimizer_steps":
                int(
                    stage5c3_sac_alpha_optimizer_steps
                ),

            "actor":
                stage5c3_c3_cpu_state_dict(
                    stage5c3_sac_actor
                ),
            ...
            "replay": {
                "obs":
                    torch.as_tensor(
                        stage5c3_sac_replay_obs
                    ).clone(),

                "action":
                    torch.as_tensor(
                        stage5c3_sac_replay_action
                    ).clone(),

                "reward":
                    torch.as_tensor(
                        stage5c3_sac_replay_reward
                    ).clone(),

                "next_obs":
                    torch.as_tensor(
                        stage5c3_sac_replay_next_obs
                    ).clone(),

                "done":
                    torch.as_tensor(
                        stage5c3_sac_replay_done
                    ).clone(),
            },
            ...
            "config": {
                ...
                "batch_size":
                    int(
                        STAGE5C3_SAC_BATCH_SIZE
                    ),

                "target_entropy":
                    float(
                        STAGE5C3_SAC_TARGET_ENTROPY
                    ),
    ...
    def stage5c3_c3_save_checkpoint(
        filename,
        completed_episode,
    ):
        ...
        torch.save(
            stage5c3_c3_checkpoint_payload(
                completed_episode
            ),
            checkpoint_path,
        )
        ...
        stage5c3_c3_checkpoint_paths.append(
            str(
                checkpoint_path
            )
        )
    ...
    def stage5c3_external_action_provider(
        ...
        """SAC stochastic actor action provider."""
        ...
        if (
            ...
            raise RuntimeError(
                "Invalid Stage5C3-C2 SAC observation"
            )
        ...
        if (
            ...
            raise RuntimeError(
                "Invalid Stage5C3-C2 SAC action"
            )
    ...
    print(
        ...
        "SAC_STOCHASTIC_ACTOR_UPDATE_SMOKE"
    )
    ...
    # =========================================================
    # STAGE5C4-B1 LEARNING PILOT
    # =========================================================

    STAGE5C4_B1_UPDATES_PER_EPISODE = int(
        os.environ.get(
            "STAGE5C4_B1_UPDATES_PER_EPISODE",
            "16",
        )
    )


    if STAGE5C4_B1_UPDATES_PER_EPISODE < 1:

        raise RuntimeError(
            "STAGE5C4_B1_UPDATES_PER_EPISODE must be >= 1"
        )
    ...
    print(
        "STAGE5C4-B1 LEARNING PILOT CONFIG"
    )
    print(
        "=================================================="
    )
    ...
    print(
        "STAGE5C4_B1_TRAIN_INCOMING_SEED_BASE=30000"
    )

    print(
        "STAGE5C4_B1_FRESH_SAC_INITIALIZATION=TRUE"
    )

    print(
        "STAGE5C4_B1_CHECKPOINT_RESUME=FALSE"
    )


    stage5c4_b1_init_checkpoint = (
        stage5c3_c3_save_checkpoint(
            "checkpoint_init.pt",
            0,
        )
    )
    ...
    for episode_idx in range(
        ...
        # =====================================================
        # STAGE5C3-C2
        # ADD TERMINAL TRANSITIONS TO SAC REPLAY BUFFER
        # =====================================================

        stage5c3_sac_obs_np = np.asarray(
            stage5c3_policy_obs,
            dtype=np.float32,
        )

        stage5c3_sac_action_np = np.asarray(
            stage5c1_action_normalized,
            dtype=np.float32,
        )

        stage5c3_sac_reward_np = np.asarray(
            stage5c1_reward,
            dtype=np.float32,
        ).reshape(
            n_envs,
        ...
        stage5c3_sac_done_np = np.asarray(
            stage5c3_terminated,
            dtype=np.float32,
        ).reshape(
            n_envs,
        ...
        if not np.all(
            stage5c3_sac_done_np
            == 1.0
        ):

            raise RuntimeError(
                "Stage5C3-C2 expected one-step terminal transitions"
            )


        # For a terminal one-step MDP, next_obs is not bootstrapped.
        # Store a zero terminal observation explicitly.
        stage5c3_sac_next_obs_np = np.zeros_like(
            stage5c3_sac_obs_np,
            dtype=np.float32,
        )


        for replay_env_id in range(
            n_envs
        ):
            ...
            stage5c3_sac_replay_action[
                replay_index
            ] = stage5c3_sac_action_np[
                replay_env_id
            ]

            stage5c3_sac_replay_reward[
                replay_index
            ] = stage5c3_sac_reward_np[
                replay_env_id
            ]

            stage5c3_sac_replay_next_obs[
                replay_index
            ] = stage5c3_sac_next_obs_np[
                replay_env_id
            ]
        ...
        print(
            "STAGE5C3_C2_REPLAY_SIZE_AFTER_EPISODE=",
            stage5c3_sac_replay_size,
        )


        # =====================================================
        # STAGE5C4-B1: RUN MULTIPLE SAC UPDATES AFTER EACH VECTOR EPISODE ONCE REPLAY >= BATCH SIZE.
        # =====================================================
        ...
        if (
            ...
            for stage5c4_b1_update_idx in range(STAGE5C4_B1_UPDATES_PER_EPISODE):
                update_rng = np.random.default_rng(
                    5701
                    + stage5c3_sac_update_steps
                )
    
                batch_indices = update_rng.integers(
                    low=0,
                    high=stage5c3_sac_replay_size,
                    size=STAGE5C3_SAC_BATCH_SIZE,
                )
                ...
                batch_next_obs = torch.as_tensor(
                    stage5c3_sac_replay_next_obs[
                        batch_indices
                    ],
                    device=device,
                ...
                # Critic target
                #
                # done == 1 for every transition in this
                # contextual-bandit / one-step MDP.
                #
                # Therefore bootstrapping is masked to zero.
                # -----------------------------------------------
                ...
                stage5c3_sac_q1_optimizer.zero_grad(
                    set_to_none=True
                )
    
                q1_loss.backward()
    
                stage5c3_c1_backward_calls += 1
    
                stage5c3_sac_q1_optimizer.step()
    
                stage5c3_sac_q1_optimizer_steps += 1
                stage5c3_c1_optimizer_steps += 1
    
    
                stage5c3_sac_q2_optimizer.zero_grad(
                    set_to_none=True
                )
                ...
                stage5c3_sac_q2_optimizer.step()
    
                stage5c3_sac_q2_optimizer_steps += 1
                stage5c3_c1_optimizer_steps += 1
                ...
                policy_action, policy_log_prob = (
                    stage5c3_sac_actor.sample(
                        batch_obs
                    )
                )
    
                policy_q1 = stage5c3_sac_q1(
                    batch_obs,
                    policy_action,
                )
    
                policy_q2 = stage5c3_sac_q2(
                    batch_obs,
                    policy_action,
                )
                ...
                stage5c3_sac_actor_optimizer.zero_grad(
                    set_to_none=True
                )
                ...
                stage5c3_sac_actor_optimizer.step()
    
                stage5c3_sac_actor_optimizer_steps += 1
                stage5c3_c1_optimizer_steps += 1
                ...
                alpha_loss = -(
                    stage5c3_sac_log_alpha
                    * (
                        policy_log_prob.detach()
                        + STAGE5C3_SAC_TARGET_ENTROPY
                    )
                ).mean()
    
    
                stage5c3_sac_alpha_optimizer.zero_grad(
                    set_to_none=True
                )
                ...
                stage5c3_sac_alpha_optimizer.step()
    
                stage5c3_sac_alpha_optimizer_steps += 1
                stage5c3_c1_optimizer_steps += 1
                ...
                stage5c3_sac_update_steps += 1
    
    
                stage5c3_sac_q1_losses.append(
                    float(
                        q1_loss
                        .detach()
                        .cpu()
                ...
                stage5c3_sac_actor_losses.append(
                    float(
                        actor_loss
                        .detach()
                        .cpu()
                ...
                stage5c3_sac_alpha_losses.append(
                    float(
                        alpha_loss
                        .detach()
                        .cpu()
                ...
                print(
                    ...
                    "TRUE_AFTER_OPTIMIZER_STEP"
                )


        # =====================================================
        # STAGE5C3-C3 FORMAL PER-EPISODE TRAINING METRICS
        # =====================================================
        ...
        stage5c3_c3_metric = {
            ...
            "alpha_loss":
                (
                    ...
                    if stage5c3_sac_alpha_losses
                    else None
                ),
        }


        stage5c3_c3_training_metrics.append(
            stage5c3_c3_metric
        )


        print(
            "STAGE5C3_C3_TRAIN_METRIC=",
            json.dumps(
                stage5c3_c3_metric,
                sort_keys=True,
            ),
        ...
        if (
            ...
            stage5c3_c3_save_checkpoint(
                (
                    "checkpoint_update_"
                    f"{stage5c3_sac_update_steps:06d}.pt"
                ),
                episode_idx + 1,
            )
    ...
    # =========================================================
    # STAGE5C3-C3 FINAL TRAINING ARTIFACTS
    # =========================================================

    stage5c3_c3_final_checkpoint = (
        stage5c3_c3_save_checkpoint(
            "checkpoint_final.pt",
            B2_EPISODES,
        )
    ...
    stage5c3_c3_metrics_json = (
        stage5c3_c3_output_dir
        / "training_metrics.json"
    )


    with stage5c3_c3_metrics_json.open(
        "w",
        encoding="utf-8",
    ) as f:
    ...
    stage5c3_c3_config_json = (
        stage5c3_c3_output_dir
        / "training_config.json"
    )


    with stage5c3_c3_config_json.open(
        "w",
        encoding="utf-8",
    ) as f:
        ...
        json.dump(
            {
                ...
                "evaluation_action":
                    "not_executed_in_training_script",

                "model21800":
                    "frozen",

                "observation":
                    "normalized_6d_physical_oracle",

                "high_level_action":
                    "normalized_4d_then_physical_mapping",
            },
            f,
            indent=2,
            sort_keys=True,
    ...
    # =========================================================
    # STAGE5C4-B0 TRAINING-AWARE POLICY BRIDGE GATE
    #
    # C1 originally required:
    #   parameters unchanged
    #   optimizer steps == 0
    #   backward calls == 0
    #
    # Those semantics are valid only for the old fixed-policy
    # integration smoke.
    #
    # In the SAC trainer, parameter changes and gradient updates
    # are REQUIRED behavior.
    # =========================================================

    stage5c4b_b0_policy_parameters_changed = bool(
        not stage5c3_c1_parameters_unchanged
    )


    stage5c4b_b0_expected_gradient_calls = int(
        4
        * stage5c3_sac_update_steps
    )


    stage5c4b_b0_gradient_update_contract = bool(
        stage5c3_sac_update_steps
        > 0

        and stage5c3_c1_optimizer_steps
        == stage5c4b_b0_expected_gradient_calls

        and stage5c3_c1_backward_calls
        == stage5c4b_b0_expected_gradient_calls

        and stage5c4b_b0_policy_parameters_changed
    )
    ...
    stage5c3_c1_gate = bool(
        ...
        and stage5c4b_b0_gradient_update_contract
    )
    ...
    print(
        "STAGE5C4-B1 TRAINING-AWARE POLICY BRIDGE RESULTS"
    )
    print(
        "=================================================="
    )
    ...
    print(
        "STAGE5C4B_B1_POLICY_CALL_COUNT=",
        "PASS"
        if stage5c3_c1_policy_call_count_ok
        else "FAIL",
    )


    print(
        "STAGE5C4B_B1_POLICY_TO_ACTION_MAPPING=",
        "PASS"
    ...
    print(
        "STAGE5C4B_B1_FRESH_RANDOM_INCOMING=",
        "PASS"
    ...
    print(
        "STAGE5C4B_B1_ACTION_FROM_POLICY_NOT_ANCHOR=",
        "PASS"
    ...
    print(
        "STAGE5C4B_B1_POLICY_PARAMETERS_CHANGED=",
        "PASS"
        if stage5c4b_b0_policy_parameters_changed
        else "FAIL",
    )


    print(
        "STAGE5C4B_B1_SAC_UPDATE_STEPS=",
        stage5c3_sac_update_steps,
    )


    print(
        "STAGE5C4B_B1_EXPECTED_GRADIENT_CALLS=",
        stage5c4b_b0_expected_gradient_calls,
    )


    print(
        "STAGE5C4B_B1_OPTIMIZER_STEPS=",
        stage5c3_c1_optimizer_steps,
    )


    print(
        "STAGE5C4B_B1_BACKWARD_CALLS=",
        stage5c3_c1_backward_calls,
    )


    print(
        "STAGE5C4B_B1_GRADIENT_UPDATE_CONTRACT=",
        "PASS"
        if stage5c4b_b0_gradient_update_contract
        else "FAIL",
    )
    ...
    print(
        "STAGE5C4B_B1_TRAINING_AWARE_GATE=",
        "PASS"
        if stage5c3_c1_gate
        else "FAIL",
    )
    ...
    if not stage5c3_c1_gate:
        ...
        raise RuntimeError(
            "Stage5C4-B0 training-aware policy bridge "
            "gate failed"
        )


    # =========================================================
    # STAGE5C3-C2 SAC TRAINER UPDATE FINAL GATE
    # =========================================================

    stage5c3_c2_actor_changed = bool(
        any(
            not torch.equal(
                parameter
                .detach()
    ...
    stage5c3_c2_target_changed = bool(
        any(
            not torch.equal(
                parameter
                .detach()
    ...
    stage5c3_c2_losses_finite = bool(
        stage5c3_sac_update_steps
        > 0
        and all(
            np.isfinite(x)
    ...
    stage5c3_c2_optimizer_steps_ok = bool(
        stage5c3_sac_update_steps
        >= 1
        and stage5c3_sac_q1_optimizer_steps
        == stage5c3_sac_update_steps
        and stage5c3_sac_q2_optimizer_steps
        == stage5c3_sac_update_steps
        and stage5c3_sac_actor_optimizer_steps
        == stage5c3_sac_update_steps
        and stage5c3_sac_alpha_optimizer_steps
        == stage5c3_sac_update_steps
    )


    stage5c3_c2_parameter_update_ok = bool(
        stage5c3_c2_actor_changed
        and stage5c3_c2_q1_changed
        and stage5c3_c2_q2_changed
        and stage5c3_c2_target_changed
    )


    stage5c3_c2_gate = bool(
        stage5c3_train_ready_gate
        and stage5c3_c1_gate
        and stage5c3_c2_replay_ok
        and stage5c3_c2_optimizer_steps_ok
        and stage5c3_c2_parameter_update_ok
        and stage5c3_c2_losses_finite
        and stage5c3_c2_alpha_finite
    )
    ...
    print(
        "STAGE5C3-C2 SAC TRAINER UPDATE RESULTS"
    )
    print(
        "=================================================="
    )
    ...
    print(
        "STAGE5C3_C2_ALGORITHM=SAC"
    )

    print(
        "STAGE5C3_C2_HIGH_LEVEL_MDP="
        "ONE_STEP_TERMINAL_CONTEXTUAL_BANDIT"
    )

    print(
        "STAGE5C3_C2_REPLAY_SIZE=",
        stage5c3_sac_replay_size,
    )

    print(
        "STAGE5C3_C2_BATCH_SIZE=",
        STAGE5C3_SAC_BATCH_SIZE,
    )

    print(
        "STAGE5C3_C2_SAC_UPDATE_STEPS=",
        stage5c3_sac_update_steps,
    )

    print(
        "STAGE5C3_C2_Q1_OPTIMIZER_STEPS=",
        stage5c3_sac_q1_optimizer_steps,
    )

    print(
        "STAGE5C3_C2_Q2_OPTIMIZER_STEPS=",
        stage5c3_sac_q2_optimizer_steps,
    )

    print(
        "STAGE5C3_C2_ACTOR_OPTIMIZER_STEPS=",
        stage5c3_sac_actor_optimizer_steps,
    )

    print(
        "STAGE5C3_C2_ALPHA_OPTIMIZER_STEPS=",
        stage5c3_sac_alpha_optimizer_steps,
    )

    print(
        "STAGE5C3_C2_ACTOR_PARAMETERS_CHANGED=",
        "PASS"
        if stage5c3_c2_actor_changed
        else "FAIL",
    )

    print(
        "STAGE5C3_C2_Q1_PARAMETERS_CHANGED=",
        "PASS"
        if stage5c3_c2_q1_changed
        else "FAIL",
    )

    print(
        "STAGE5C3_C2_Q2_PARAMETERS_CHANGED=",
        "PASS"
        if stage5c3_c2_q2_changed
        else "FAIL",
    )

    print(
        "STAGE5C3_C2_TARGET_CRITIC_UPDATED=",
        "PASS"
        if stage5c3_c2_target_changed
        else "FAIL",
    )

    print(
        "STAGE5C3_C2_LOSSES_FINITE=",
        "PASS"
        if stage5c3_c2_losses_finite
        else "FAIL",
    )

    print(
        "STAGE5C3_C2_FINAL_ALPHA=",
        float(
            stage5c3_sac_log_alpha
            .exp()
            .detach()
    ...
    print(
        "STAGE5C3_C2_TERMINAL_BOOTSTRAP_MASK="
        "ZERO"
    )

    print(
        "STAGE5C3_C2_REPLAY_CONTRACT=",
        "PASS"
        if stage5c3_c2_replay_ok
        else "FAIL",
    )

    print(
        "STAGE5C3_C2_OPTIMIZER_UPDATE_CONTRACT=",
        "PASS"
        if stage5c3_c2_optimizer_steps_ok
        else "FAIL",
    )
    ...
    # =========================================================
    # STAGE5C3-C3-B FORMAL TRAINER INFRASTRUCTURE GATE
    # =========================================================

    stage5c3_c3_metric_count_ok = bool(
        len(
            stage5c3_c3_training_metrics
        )
        == B2_EPISODES
    ...
    stage5c3_c3_metrics_finite = bool(
        stage5c3_c3_metric_count_ok
        and all(
            np.isfinite(
                metric[
                    "contact_rate"
            ...
            for metric
            in stage5c3_c3_training_metrics
        )
    )


    stage5c3_c3_checkpoint_files_ok = bool(
        stage5c3_c3_final_checkpoint.is_file()
        and stage5c3_c3_latest_checkpoint.is_file()
        and stage5c3_c3_metrics_json.is_file()
        and stage5c3_c3_config_json.is_file()
    )


    stage5c3_c3_roundtrip = torch.load(
        stage5c3_c3_final_checkpoint,
        map_location="cpu",
        weights_only=False,
    )
    ...
    stage5c3_c3_required_checkpoint_keys = {
        "schema",
        "completed_episodes",
        "num_envs",
        "replay_size",
    ...
    stage5c3_c3_checkpoint_schema_ok = bool(
        stage5c3_c3_required_checkpoint_keys
        .issubset(
            set(
                stage5c3_c3_roundtrip.keys()
            )
        )
        and stage5c3_c3_roundtrip[
            "schema"
        ]
        == "STAGE5C3_C3_SAC_PILOT_V1"
        and int(
            stage5c3_c3_roundtrip[
                "completed_episodes"
            ]
        )
        == B2_EPISODES
    ...
    stage5c3_c3_replay_checkpoint_ok = bool(
        set(
            stage5c3_c3_roundtrip[
                "replay"
            ].keys()
    ...
    stage5c3_c3_optimizer_state_ok = bool(
        stage5c3_c3_roundtrip[
            "actor_optimizer"
        ][
            "state"
    ...
    stage5c3_c3_gate = bool(
        stage5c3_c2_gate
        and stage5c3_c3_metric_count_ok
        and stage5c3_c3_metrics_finite
        and stage5c3_c3_checkpoint_files_ok
        and stage5c3_c3_checkpoint_schema_ok
        and stage5c3_c3_replay_checkpoint_ok
        and stage5c3_c3_optimizer_state_ok
        and stage5c3_sac_update_steps
        > 0
    )
    ...
    print(
        "STAGE5C3-C3-B FORMAL TRAINER INFRASTRUCTURE RESULTS"
    )
    print(
        "=================================================="
    )
    ...
    print(
        "STAGE5C3_C3_METRIC_EPISODE_COUNT=",
        len(
            stage5c3_c3_training_metrics
        ),
    )
    ...
    print(
        "STAGE5C3_C3_METRICS_FINITE=",
        "PASS"
        if stage5c3_c3_metrics_finite
        else "FAIL",
    )

    print(
        "STAGE5C3_C3_CHECKPOINT_FILES=",
        "PASS"
        if stage5c3_c3_checkpoint_files_ok
        else "FAIL",
    )

    print(
        "STAGE5C3_C3_CHECKPOINT_SCHEMA=",
        "PASS"
        if stage5c3_c3_checkpoint_schema_ok
        else "FAIL",
    )

    print(
        "STAGE5C3_C3_REPLAY_CHECKPOINT=",
        "PASS"
        if stage5c3_c3_replay_checkpoint_ok
        else "FAIL",
    )

    print(
        "STAGE5C3_C3_OPTIMIZER_STATE_CHECKPOINT=",
        "PASS"
        if stage5c3_c3_optimizer_state_ok
        else "FAIL",
    )

    print(
        "STAGE5C3_C3_CHECKPOINT_ROUNDTRIP="
        "PASS"
        if stage5c3_c3_checkpoint_schema_ok
        else "FAIL"
    )
    ...
    print(
        "STAGE5C3_C3_TOTAL_TRANSITIONS=",
        stage5c3_sac_replay_size,
    )

    print(
        "STAGE5C3_C3_TOTAL_SAC_UPDATES=",
        stage5c3_sac_update_steps,
    )

    print(
        "STAGE5C3_C3_EVALUATION_UPDATES=0"
    )

    print(
        "STAGE5C3_C3_EVALUATION_STATUS="
        "NOT_RUN_SEPARATE_NEXT_STAGE"
    )

    print(
        "NEW_HIGH_LEVEL_RL_TRAINING_STARTED=TRUE"
    )

    print(
        "STAGE5C3C3_FORMAL_TRAINER_INFRASTRUCTURE_SMOKE=",
        "PASS"
        if stage5c3_c3_gate
        else "FAIL",
    )
    ...
    if not stage5c3_c3_gate:

        raise RuntimeError(
            "Stage5C3-C3-B formal trainer "
            "infrastructure gate failed"
        )


    if not stage5c3_c2_gate:

        raise RuntimeError(
            "Stage5C3-C2 SAC update gate failed"
        )
